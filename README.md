...
배고프면 진수에게는 관리자만 맛집을 등록하고, 방문자는 공개 링크에서 맛집을 검색해 볼 수 있는 Next.js MVP입니다.
...
저장 리스트에서 가져온 장소는 네이버 장소에 등록된 공식 썸네일을 최대 3개 확인한 뒤 첫 번째 사진부터 Cloudflare R2에 자동 복사합니다. 자동 저장에 실패한 장소는 관리자 수정 화면에서 공식 후보를 다시 선택할 수 있습니다. 직접 등록 화면에서는 이미지를 최대 3장 업로드하거나 장소 선택 후 자동으로 표시되는 네이버 추천 이미지 후보를 선택할 수 있습니다. 공개 상세 화면에는 저장된 이미지 3장까지 갤러리로 표시합니다.

이미지 기능과 방문 상태 기능을 기존 Supabase 프로젝트에 추가할 때는 변경된 `supabase/schema.sql`을 SQL Editor에서 다시 실행해 `image_paths`, `image_source_url`, `image_credit`, `image_candidates`, `has_visited` 컬럼과 인덱스를 추가해 주세요. 기존 식당은 방문 여부를 확인할 수 없으므로 `진수가 아직 안 가봤어요`로 시작하며, 관리자 수정 화면에서 `진수가 가본 식당`을 체크하면 공개 화면에 자동으로 `진수가 가봤어요` 태그가 표시됩니다. 기존 장소 사진을 보완하려면 관리자에서 같은 저장 리스트 링크를 다시 읽은 뒤 `기존 사진 보완`을 누르면 됩니다. 기존에 직접 업로드한 사진은 덮어쓰지 않습니다.

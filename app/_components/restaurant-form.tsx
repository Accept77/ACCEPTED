...
import { importImageFromNaver } from "@/app/actions/images";
import { createRestaurant, updateRestaurant } from "@/app/actions/restaurants";
import { createClient } from "@/lib/supabase/client";
import { RESTAURANT_CATEGORIES, STORAGE_BUCKET } from "@/lib/constants";
...
export function RestaurantForm({
  ...
  async function uploadImage(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!isConfigured) {
      setFormError("이미지를 업로드하려면 Supabase를 먼저 설정해 주세요.");
      return;
    }
    ...
    try {
      ...
      if (failed) {
        if (uploaded.length > 0) {
          await supabase.storage.from(STORAGE_BUCKET).remove(uploaded.map(({ path }) => path));
        }
  ...
  return (
    <form className="space-y-8" onSubmit={handleSubmit}>
      <section className="rounded-[1.75rem] border border-slate-200/80 bg-white p-5 shadow-[0_18px_55px_-35px_rgba(20,32,51,0.35)] sm:p-7">
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <input
            className="h-13 flex-1 rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-[#8eaff4] focus:bg-white focus:ring-4 focus:ring-[#edf3ff]"
          ...
          />
          <button
            className="h-13 rounded-2xl bg-[#142033] px-5 text-sm font-bold text-white transition hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-50"
        ...
        {searchResults.length > 0 ? (
          <div className="mt-4 divide-y divide-slate-100 overflow-hidden rounded-2xl border border-slate-200">
            {searchResults.map((place) => (
              <button
                className="block w-full px-4 py-4 text-left transition hover:bg-[#f8faff]"
                key={place.id}
      ...
      <section className="rounded-[1.75rem] border border-slate-200/80 bg-white p-5 shadow-[0_18px_55px_-35px_rgba(20,32,51,0.35)] sm:p-7">
        ...
        <div className="mt-6 grid gap-5 sm:grid-cols-2">
          <label className="sm:col-span-2">
            ...
            <input className={inputClass} onChange={(event) => updateText("name", event.target.value)} required value={form.name} />
          </label>

          <label>
            <span className="text-xs font-bold text-slate-600">카테고리</span>
            <select className={inputClass} onChange={(event) => updateText("category", event.target.value)} value={form.category}>
          ...
          <label>
            ...
            <input className={inputClass} onChange={(event) => updateText("area", event.target.value)} placeholder="예: 성수, 연남" value={form.area} />
          </label>

          <label className="sm:col-span-2">
            <span className="text-xs font-bold text-slate-600">주소</span>
            <input className={inputClass} onChange={(event) => updateText("address", event.target.value)} required value={form.address} />
          </label>

          <label className="sm:col-span-2">
            <span className="text-xs font-bold text-slate-600">추천 메모</span>
            <textarea
              className="mt-2 min-h-32 w-full resize-y rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm leading-6 text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-[#8eaff4] focus:ring-4 focus:ring-[#edf3ff]"
          ...
          <label className="sm:col-span-2">
            ...
            <input className={inputClass} onChange={(event) => updateText("naverUrl", event.target.value)} required type="url" value={form.naverUrl} />
          </label>
        </div>
      </section>
      ...
      <section className="rounded-[1.75rem] border border-slate-200/80 bg-white p-5 shadow-[0_18px_55px_-35px_rgba(20,32,51,0.35)] sm:p-7">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold tracking-[-0.05em] text-slate-900">대표 사진을 추가하세요.</h2>
          <p className="mt-2 text-sm text-slate-500">직접 업로드하거나 네이버 이미지 후보를 확인한 뒤 선택할 수 있습니다.</p>
        </div>
        ...
        <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-start">
          ...
          <div className="flex min-w-40 flex-col items-start gap-3">
            ...
            <label className={`cursor-pointer rounded-xl bg-[#142033] px-4 py-3 text-xs font-bold text-white transition hover:bg-slate-700 ${imagePreviews.length >= MAX_IMAGES ? "pointer-events-none opacity-50" : ""}`}>
              ...
              <input accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={isUploading || isImportingImage || !isConfigured || imagePreviews.length >= MAX_IMAGES} multiple onChange={uploadImage} type="file" />
            </label>
              ...
              {form.imageCandidates.map((candidate, index) => {
                ...
                return (
                  <button
                    ...
                    disabled={isImportingImage || isUploading || !isConfigured || imagePreviews.length >= MAX_IMAGES || isSelected}
                    key={candidate}
                    onClick={() => {
              ...
              {imageCandidates.map((candidate) => {
                ...
                return (
                  <button
                    ...
                    className={`overflow-hidden rounded-2xl border text-left transition hover:-translate-y-0.5 hover:shadow-md disabled:cursor-wait disabled:opacity-60 ${isSelected ? "border-[#2f6fed] ring-2 ring-[#dce8ff]" : "border-slate-200"}`}
                    disabled={isImportingImage || isUploading || imagePreviews.length >= MAX_IMAGES || isSelected}
                    key={candidate.id}
                    onClick={() => {
      ...
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
        <Link className="flex h-13 items-center justify-center rounded-2xl border border-slate-200 bg-white px-5 text-sm font-bold text-slate-600 transition hover:border-slate-300" href="/admin">
          취소
        </Link>
        <button className="h-13 rounded-2xl bg-[#2f6fed] px-7 text-sm font-bold text-white shadow-lg shadow-blue-500/15 transition hover:bg-[#255ac8] disabled:cursor-not-allowed disabled:opacity-50" disabled={isSaving || isUploading} type="submit">
